...
err_t rimuovi(array_circolare_t &a, position p, int &out) {
	...
	if (p==HEAD) {
		...
		// clog << (a.start+a.dim)%MAX_DIM << endl;
		out = a.v[(a.start+a.dim-1)%MAX_DIM];
	}
	
	a.dim--;
	
	clog << "Rimosso ele: " << out << endl;
...
err_t inserisci(coda &c, int ele) {
	return inserisci(c.a, HEAD, ele);
}

err_t rimuovi(coda &c, int &ele) {
	return rimuovi(c.a, TAIL, ele);
}

void test() {
	cout << "Testing coda" << endl;
